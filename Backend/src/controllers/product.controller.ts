...
// ─── Frontend-easy indexed image helper: image[0] file (local/storage), image[0].url (online), image[0].isMain bool ───
// file = local/storage upload (multer -> /uploads/products/images/<filename>, sharp compressed)
// url = online image URL (saved as-is, e.g. https://cdn.com/img.jpg, deleteFile skips http)
// If BOTH file and url are sent for the SAME index, file wins and url is ignored (as requested)
...
function parseIndexedImages(body: any, files: Express.Multer.File[]): { imageUrl: string; isMain: boolean; sortOrder: number }[] | null {
 ...
 // file (local/storage) takes precedence over url (online) for same index — as requested
 if (e.file) imageUrl = `/uploads/products/images/${e.file.filename}`; // local file saved via multer diskStorage
 else if (e.url) imageUrl = e.url; // online URL saved as-is
 ...
 // sort by sortOrder then idx, then re-normalize contiguous like 
...
function validateProductPayload(body: any) {
 ...
 // Supports both JSON array and multipart form-data JSON-stringified array ( pattern: tryParse)
 ...
 // images optional — also handles multipart JSON string (: safeParse) and allows empty when files are uploaded
...
export const createProduct = asyncHandler(async (req: Request, res: Response) => {
 ...
 // mainImageUrl for quick card lookup: use isMain image, fallback to first (order 0) like 
 ...
 // best: on failure, delete any uploaded files that were already written to disk
...
export const getProductById = asyncHandler(async (req: Request, res: Response) => {
 ...
 // best: sort images by order and expose hasImages flag, main is order 0
...
// ─── best scenarios: media management after creation ───
...
// Remove image by its sort_order (or id) — mirrors removeMedia by order
export const removeProductImage = asyncHandler(async (req: Request, res: Response) => {
 ...
 // re-normalize remaining orders to 0,1,2... like 
...
// Reorder images — mirrors reorderMedia (currentOrder -> newOrder)
